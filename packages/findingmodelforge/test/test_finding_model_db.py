...
async def test_create_and_get_finding_model(db_init):
    ...
    finding_model = FindingModelDb(name="Test Model", description="This is a test model", attributes=attributes)
    ...
    assert count == 1
    retrieved_model = await FindingModelDb.find_one(FindingModelDb.name == "Test Model")
