...
The `humanize` filter exists and is registered, but most templates use `.strftime()` instead of the more user-friendly
humanize filter.
...
**Solution**: Replace `{{ timestamp.strftime(...) }}` with `{{ timestamp | humanize }}`
